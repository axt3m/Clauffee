...
extension Strings {
    static let en = Strings(
        ...
        obIntro: "Start Brew keeps your Claude Code sessions alive with the lid closed. Three one-time checks:",
        ...
        obNote: { h in "Every new brew starts with the \(h) h limit on and stops by itself when the last Claude Code session ends — both adjustable in Settings ⚙︎. When you reopen the lid, CaffeLatte sums up or offers to stop." },
...
extension Strings {
    static let fr = Strings(
        ...
        obIntro: "Le café garde tes sessions Claude Code vivantes capot fermé. Trois vérifs — une seule fois :",
        ...
        obNote: { h in "Chaque nouvelle infusion démarre avec la limite de \(h) h activée et s'arrête toute seule quand la dernière session Claude Code se termine — les deux se changent dans les Réglages ⚙︎. À la réouverture du capot, CaffeLatte te résume ou te propose d'arrêter." },
...
extension Strings {
    static let ru = Strings(
        ...
        obIntro: "Заварка держит сессии Claude Code живыми при закрытой крышке. Три проверки — один раз:",
        ...
        obNote: { h in "Каждая новая заварка стартует с лимитом \(h) ч и сама останавливается, когда завершается последняя сессия Claude Code — обе настройки меняются в Настройках ⚙︎. Когда откроешь крышку, CaffeLatte подведёт итог или предложит остановить." },
